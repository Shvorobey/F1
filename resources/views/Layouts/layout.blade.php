...
<head>
    ...
    <meta name="keywords" content="@yield('meta_keywords','F1, Odessa, formula 1, club, формула 1, Ф1, клуб, Одесса')">
    <meta name="description" content="@yield('meta_description','Odessa Formula 1 Club, Одесский Клуб формулы 1')">
                        ...
                        <ul id="menu-menu" class="level-1">
                            ...
                            @if(Auth::check() && Auth::user()->role >= 1)
                                <li>
                                    ...
                                    <ul class="sub-menu">
                                        ...
                                        <li><a href="{{route('posts_all')}}">Новости</a></li>
                            ...
                            <div id="topbar_login" class="not_logged_in">
                                ...
                                @if(\Illuminate\Support\Facades\Auth::check())
                                    <li>
                                        <a href="{{ route('logout') }}"
                                           onclick="event.preventDefault(); document.getElementById('logout-form').submit()"><strong
                                                style="color: greenyellow">Log out</strong></a>
                            ...
                            <div class="textwidget">
                                ...
                                @foreach($networks->getSocialNetworks() as $network)
                                    <div class="socials-square-small {{$network->key}}-small default">
                                        <a href="{{$network->link}}" title="{{$network->name}}" target="_blank"
                                           class="socials-square-small default {{$network->key}}">{{$network->name}}</a>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="footer-widgets-area with-sidebar-right">
                            <div class="widget-first widget span10 widget_text">
                                <div class="textwidget">
                                    <b>Formula 1 Odessa Club</b> - новое движение одесских фанатов формулы 1 и
                                    автоспорта.
                                    Мы предлагаем объединиться всем ценителям королевских автогонок нашего региона.
                                    Ответ на вопрос о том, где смотреть формулу 1 в Одессе очень прост - вместе с нашим
                                    клубом F1 Odessa club.
                                    На все гонки любого гран при в сезоне организовывается отдельный сбор клуба в одном
                                    из спорт баров Одессы,
                                    в котором обязательным условием является наличие прямого эфирa с автодрома. Помимо
                                    формулы 1 практикуются
                                    просмотры других серий автогонок таких как мото гп, наскар, RОC (race of champions)
                                    и многое другое.
                                    В наших чатах предлагается принять участие в конкурсах прогнозов для этого перед
                                    гонкой каждый может подать
                                    свой прогноз и, в случае хорошего результата, получит интересный сувенир. Регулярно
                                    клуб организовывает
                                    турниры по картингу на одном из украинских картинговых автодромов. Однако не только
                                    автоспортом живёт наш клуб.
                                    Во время летних перерывов члены клуба собираются на пляжные пикники и осуществляют
                                    вылазки в красивые
                                    места Украины. В зимнее межсезонье проводятся отдельные встречи на просмотр
                                    интересных гонок прошлых лет
                                    или просто обсуждение текущих моментов в Формуле-1. Если вам не безразличен мир
                                    формулы 1 и автоспорта,
                                    ...
                                    <ul id="menu-socialize" class="menu">
                                        @foreach($networks->getSocialNetworks() as $network)
                                            <li class="menu-item menu-item-type-custom">
                                                <a href="{{$network->link}}" target="_blank">{{$network->name}}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
